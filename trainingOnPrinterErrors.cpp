...
// would be aaabbbbhaijjjm meaning that the printer used three times color a, four times color b, one time color h then one time color a...
...
std::string Printer::printerError(const std::string &s) {
  //a denominator variable equal to the length of the input string
  int denom = s.length();
  //a numerator variable eqaul to the number of letters greater than 'm'
  int numer = 0;
  ...
  std::string answer = "";
  answer += std::to_string(numer);
  answer += '/';
  answer += std::to_string(denom);
